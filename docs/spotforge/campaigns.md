...
3. Konzept, Skript und Storyboard prüfen und freigeben. fal lokal konfigurieren. Motive für den Batch auswählen, Vorschau auf Blockaden und Auftragsanzahl prüfen und die kostenpflichtige Produktion ausdrücklich bestätigen. Die aktuelle Integration unterstützt Kling 2.5 Turbo Pro mit 5 oder 10 Sekunden und Startbild bzw. Start+Endbild. Sie ist kein generischer Adapter für beliebige fal-Modelle.
...
Der Batch führt Video-KI-Aufträge aus; die finale Freigabe und der Remotion-Export bleiben pro Fassung bewusst kontrollierbar. Sounddesign, manuelle Display-Einsätze und feinere Comedy-Pausen bleiben redaktionelle Arbeit. Ein Chat-Verweis allein enthält keine herunterladbaren JSON-/Bildpakete: Originaldateien müssen separat bereitgestellt werden. Diese Version erzeugt aus einer freien Werbeidee nicht automatisch ein vollständiges Drehbuch oder Grenzbilder.
...
- `GET /api/campaigns/{id}/export` liefert `{document,warnings}`; für einen späteren Import nur `document` verwenden.
