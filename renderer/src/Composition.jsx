...
import {AbsoluteFill, Audio, Img, OffthreadVideo, Sequence, cancelRender, continueRender, delayRender, useCurrentFrame, useVideoConfig} from 'remotion';
...
export function VideoComposition({manifest: m}) {
  ...
  const brand = m.brand || {};
  const fontUrl = brand.font_asset_id ? m.assets[brand.font_asset_id]?.url : null;
  const fontFamily = fontUrl ? `SpotForge-${m.project_id}-${brand.version}` : brand.font_family || 'Arial';
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      if (brand.font_asset_id && !fontUrl) throw new Error('Required brand font is missing');
      if (fontUrl) {
      ...
      if (!cancelled) { setFontReady(true); continueRender(fontHandle); }
    };
    load().catch(error => { if (!cancelled) cancelRender(error); });
    return () => { cancelled = true; };
  }, [fontUrl, fontFamily, fontHandle, brand.font_asset_id]);
  if (!fontReady) return null;
  const color = brand.colors || {};
  const margin = Math.round(Math.min(width, height) * (brand.safe_margin ?? .07));
  const fontSize = Math.round(Math.min(width, height) * .05);
